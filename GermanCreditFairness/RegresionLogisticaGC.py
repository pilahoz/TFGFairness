...
gslog_reg.fit(GermanCredit.processtrain_x, GermanCredit.train_y) #Entrenamos el algoritmo de machine learning
...
#Calculamos el score
score = gslog_reg.score(GermanCredit.processvalid_x, GermanCredit.valid_y) #Devuelve la precisión media en los datos de prueba y las etiquetas dadas
print("The score for this model is ", score)

#Matriz de confusion
pred_y = gslog_reg.predict(GermanCredit.processvalid_x) #Prediccion
...
y_pred_proba = gslog_reg.predict_proba(GermanCredit.processvalid_x)[:, 1] # Obtenemos las probabilidades de predicción del modelo para el conjunto de prueba
...
p1_hat = gslog_reg.predict_proba(GermanCredit.grupo_h) #probabilidad del grupo_m
p2_hat = gslog_reg.predict_proba(GermanCredit.grupo_m) #probabilidad del grupo_h

#-------PARIDAD-------
umbral_hp, umbral_mp =Fairness.umbralparidad(GermanCredit.grupo_h, GermanCredit.grupo_m, p1_hat, p2_hat, min_umbral)
print("umbra h:",umbral_hp, "umbral m:", umbral_mp)

#columna de prob que si reincidan
ind1 = p1_hat[:,1]>umbral_hp
ind2 = p2_hat[:,1]>umbral_mp
...
umbral_hip, umbral_mip = Fairness.umbralprobabilidades(GermanCredit.grupo_h, GermanCredit.grupo_m, p1_hat, p2_hat, gslog_reg, GermanCredit.valid_yaux1, GermanCredit.valid_yaux2, GermanCredit.valid_yaux, min_umbral)
#umbral_hip, umbral_mip = Fairness.umbralprobabilidades(GermanCredit.grupo_m,GermanCredit.grupo_h, p2_hat, p1_hat,  gslog_reg, GermanCredit.valid_yaux2, GermanCredit.valid_yaux1,GermanCredit.valid_yaux, min_umbral)
print("umbra h:",umbral_hip, "umbral m:", umbral_mip)

ind1ip = p1_hat[:,1]>umbral_hip #columna de prob que si reincidan
ind2ip = p2_hat[:,1]>umbral_mip
...
umbral_hio, umbral_mio = Fairness.umbraloportunidades(GermanCredit.grupo_h, GermanCredit.grupo_m, p1_hat, p2_hat, gslog_reg, GermanCredit.valid_yaux1, GermanCredit.valid_yaux2, GermanCredit.valid_yaux, min_umbral)
print("umbra h:",umbral_hio, "umbral m:", umbral_mio)

ind1io = p1_hat[:,1]>umbral_hio #columna de prob que si reincidan
ind2io = p2_hat[:,1]>umbral_mio
...
#-------IMPACTO DESIGUAL-------
umbral_hid, umbral_mid = Fairness.umbralimpactodesigual(GermanCredit.grupo_h, GermanCredit.grupo_m, p1_hat, p2_hat, gslog_reg, GermanCredit.valid_yaux1, GermanCredit.valid_yaux2, GermanCredit.valid_yaux, min_umbral)
print("umbra h:",umbral_hid, "umbral m:", umbral_mid)

ind1id = p1_hat[:,1]>umbral_hid #columna de prob que si reincidan
ind2id = p2_hat[:,1]>umbral_mid
